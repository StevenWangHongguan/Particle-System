...
extern float GRAVITY = 1.0f; // 重力为10
...
vec3 fireWorksPos(0.0f, -1.0f, -2.0f);
vec3 firePos(-10.0f, -5.0f, 5.0f);
vec3 snowPos(15.0f, 10.0f, 5.0f);
vec3 startPos(0.0f, 40.0f, -5.0f);
vec3 galaxyPos(0.0f, -1.0f, 10.0f);
vec3 smokePos(-5.0, -1.0, -2.0);
...
int main()
{
	...
	glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE); // uncomment this statement to fix compilation on OS X
		...
		// --------------------
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "ParticleSystem", NULL, NULL);
	...
	//textureID_3 = loadTexture("./resource/texture/particleSnow2.png");
	...
	//ParticleSystem fireWorks(500, 2, 0, 3, fireWorksTexture); //166, 2, -1, 5, 
	//ParticleSystem fire(200, 3.6, 0.15, 3.5, fireTexture); // 150/200 3.6 0.15 2.2/3.5
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// render particles 
		// 20万粒子 多线程相比单线程 从20帧提升到30帧
		for (int i = 0; i < ParticleSystemMaster::particleSystems.size(); i++)
		{
			auto bind_func = std::bind(ParticleSystemMaster::GenerateParticles, i, camera);
			thread_pool.add_task(bind_func);
		}
		for (int i = 0; i < ParticleSystemMaster::particleSystems.size(); i++)
		{
			ParticleSystemMaster::particleSystems[i].render(camera);
			//ParticleSystemMaster::particleSystems[i].Start(ParticleSystemMaster::particleSystemsPositions[i], camera);
		}

		//particleSystem start
		/*if (fireWorksStart)
		{
			fireWorks.Start(fireWorksPos,camera);
		}

		if (fireStart) 
		{
			fire.Start(firePos, camera);
		}
		
		if (snowStart)
		{
			snow.Start(snowPos, camera);
		}

		if (starStart)
		{
			star.Start(startPos, camera);
		}

		if (galaxyStart)
		{
			galaxy.Start(galaxyPos, camera);
		}

		if (smokeStart)
		{
			smoke.Start(smokePos, camera);
		}*/
